...
const els = {
  chooseFolderBtn: $('#chooseFolderBtn'), chooseOutputBtn: $('#chooseOutputBtn'), chooseOutputBtn2: $('#chooseOutputBtn2'), resetOutputBtn: $('#resetOutputBtn'),
  ...
  nameMode: $('#nameMode'), namePrefix: $('#namePrefix'), sequenceStart: $('#sequenceStart'), sequenceDigits: $('#sequenceDigits'), outputDestinationText: $('#outputDestinationText'),
...
function updateSummary(){
  ...
  els.resetOutputBtn.classList.toggle('hidden',!state.outputExplicit); updateSelectionUI();
}

async function chooseOutputFolder(){
  try{const h=await window.showDirectoryPicker({mode:'readwrite',id:'sony-video-rotator-output'});state.outputDirHandle=h;state.outputExplicit=true;updateSummary();els.processMessage.textContent=`ปลายทาง Export: ${h.name}`;}
  catch(e){if(e?.name!=='AbortError')alert(`เลือกปลายทางไม่สำเร็จ: ${e.message||e}`);}
}
function resetOutputFolder(){state.outputDirHandle=null;state.outputExplicit=false;updateSummary();els.processMessage.textContent='ปลายทางกลับเป็นโฟลเดอร์ Output ภายในต้นฉบับ';}
...
async function showPreview(index, { autoplay = false } = {}){
  ...
  els.previewMeta.textContent=`${item.name} · ${formatBytes(item.size)} · metadata ${rotationLabel(item.currentRotation)}`; const vis=visibleIndexes();const pos=vis.indexOf(state.previewIndex);els.previewIndex.textContent=pos>=0?`${pos+1} / ${vis.length}`:`${state.previewIndex+1} / ${state.files.length}`;els.clipName.textContent=item.name;
...
function sanitizedPrefix(){return (els.namePrefix.value.trim()||'RUN').replace(/[\\/:*?"<>|]+/g,'_').replace(/\s+/g,'_').slice(0,40);}
function planOutputNames(queue,retry=false){
  const mode=els.nameMode.value,start=Math.max(0,Math.floor(Number(els.sequenceStart.value)||1)),digits=Number(els.sequenceDigits.value)||3,prefix=sanitizedPrefix();
  queue.forEach((item,i)=>{if(retry&&item.plannedOutputName)return;const ext=/\.mov$/i.test(item.name)?'.mov':'.mp4';if(mode==='sequence')item.plannedOutputName=`${prefix}_${String(start+i).padStart(digits,'0')}${ext}`;else{const base=item.name.replace(/\.[^.]+$/,'');item.plannedOutputName=`${base}_cut${ext}`;}});
}
async function resolveOutputHandle(){if(state.outputExplicit&&state.outputDirHandle)return state.outputDirHandle;if(!state.dirHandle)throw new Error('ยังไม่ได้เลือกโฟลเดอร์ต้นฉบับ');state.outputDirHandle=await state.dirHandle.getDirectoryHandle('Output',{create:true});return state.outputDirHandle;}
...
// Source/output
els.chooseFolderBtn.addEventListener('click',loadFolder);els.chooseOutputBtn.addEventListener('click',chooseOutputFolder);els.chooseOutputBtn2.addEventListener('click',chooseOutputFolder);els.resetOutputBtn.addEventListener('click',resetOutputFolder);els.reviewModeBtn.addEventListener('click',toggleReviewMode);
// Presets
...
[els.nameMode,els.namePrefix,els.sequenceStart,els.sequenceDigits].forEach(el=>el.addEventListener('change',()=>{state.files.forEach(f=>f.plannedOutputName=null);}));
