...
function trimText(item) {
  if (!item.trimEnabled || !Number.isFinite(item.duration)) return 'ทั้งคลิป';
...
function renderFiles() {
  ...
  els.fileList.innerHTML = state.files.map((item, i) => `
    <button class="file-row ${i === state.previewIndex ? 'active' : ''}" data-index="${i}" type="button">
...
async function loadFolder() {
  try {
    ...
    await showPreview(0);
    els.processMessage.textContent = 'พร้อมแล้ว — เลือกช่วงตัดแต่ละคลิปแล้วกด Export ทั้งหมด';
  } catch (error) {
...
function setTrimControlsEnabled(enabled) {
  ...
  for (const el of [els.trimEnabled, els.startRange, els.endRange, els.setStartBtn, els.setEndBtn, els.jumpStartBtn, els.jumpEndBtn, els.clearTrimBtn, els.previewSelectionBtn, els.trimStartHandle, els.trimEndHandle]) {
...
function updateTrimUI() {
  ...
  els.trimEnabled.checked = !!item.trimEnabled;
  ...
  els.clipExportRange.textContent = item.trimEnabled ? `${formatTime(start)} → ${formatTime(end)}` : 'ทั้งคลิป';
  els.clipTrimStatus.textContent = item.trimEnabled ? '✓ ตัดแบบ Lossless' : 'ไม่ตัด';
  updateStoryTimeline();
}
...
function outputName(item) {
  if (!item.trimEnabled) return item.name;
  ...
  try {
    ...
    for (let i = 0; i < state.files.length; i++) {
      ...
      try {
        ...
        if (item.trimEnabled && Number.isFinite(item.duration)) {
        ...
        await writeRotatedSource(source, outHandle, state.rotation, within => setOverallProgress(i, within));
    ...
    if (state.cancelRequested) {
      ...
      els.processMessage.textContent = `เสร็จแล้ว · สำเร็จ ${completed} จาก ${state.files.length} ไฟล์ · อยู่ใน Output/`;
    }
...
els.trimEnabled.addEventListener('change', () => {
  const item = currentItem();
  if (!item) return;
  item.trimEnabled = els.trimEnabled.checked;
  updateTrimUI();
  renderFiles();
});
...
els.clearTrimBtn.addEventListener('click', () => {
  ...
  item.trimStart = 0; item.trimEnd = item.duration; item.trimEnabled = false;
  state.previewingSelection = false;
  updateTrimUI(); renderFiles();
});
