...
function applyPreviewRotation() {
  ...
  els.previewVideo.style.transform = `rotate(${r === 270 ? -90 : r}deg)`;
  const sideways = r === 90 || r === 270;
  els.previewVideo.style.maxWidth = sideways ? '68vh' : '80%';
  els.previewVideo.style.maxHeight = sideways ? '80vw' : '68vh';
}
...
for (const input of document.querySelectorAll('input[name="rotation"]')) {
  input.addEventListener('change', () => {
    state.rotation = Number(input.value);
