...
async function readMediaDuration(item, timeoutMs = 15000) {
  ...
  try {
    ...
    item.videoWidth = Number(video.videoWidth) || item.videoWidth || null;
    item.videoHeight = Number(video.videoHeight) || item.videoHeight || null;
...
async function getImageDimensions(file){
  ...
  return await new Promise((resolve,reject)=>{const u=URL.createObjectURL(file),im=new Image();im.onload=()=>{resolve({width:im.naturalWidth||1,height:im.naturalHeight||1});URL.revokeObjectURL(u);};im.onerror=()=>{URL.revokeObjectURL(u);reject(new Error('อ่านรูปภาพไม่ได้'));};im.src=u;});
}
async function addWatermarkFile(file,opts={}){
...
async function addKeitaWatermark(){
  ...
  catch(e){alert(`เพิ่มโลโก้ KEITA ไม่สำเร็จ: ${e.message||e}`);}
}
function deleteSelectedWatermark(){const wm=selectedWatermark();if(!wm)return;URL.revokeObjectURL(wm.url);const idx=state.watermarks.findIndex(w=>w.id===wm.id);state.watermarks.splice(idx,1);state.selectedWatermarkId=state.watermarks[Math.min(idx,state.watermarks.length-1)]?.id??null;if(!state.watermarks.length){state.watermarkEnabled=false;els.watermarkEnabled.checked=false;}renderWatermarks();renderWatermarkPanel();updateSelectionUI();}
function duplicateSelectedWatermark(){const wm=selectedWatermark();if(!wm)return;const id=++state.watermarkSeq;const copy={...wm,id,name:`${wm.name.replace(/(\.[^.]+)?$/,'')}_copy${wm.name.match(/\.[^.]+$/)?.[0]||''}`,url:URL.createObjectURL(wm.file),x:clamp(wm.x+.04,0,1),y:clamp(wm.y+.04,0,1)};state.watermarks.push(copy);state.selectedWatermarkId=id;renderWatermarks();renderWatermarkPanel();updateSelectionUI();}
function syncWatermarkLayerBounds(){
  if(!els.watermarkLayer||!els.previewVideo||els.previewVideo.style.display==='none'){if(els.watermarkLayer){els.watermarkLayer.style.width='0px';els.watermarkLayer.style.height='0px';}return;}
  const stage=els.previewVideo.closest('.video-stage');if(!stage)return;const sr=stage.getBoundingClientRect(),vr=els.previewVideo.getBoundingClientRect();
  if(vr.width<2||vr.height<2)return;els.watermarkLayer.style.left=`${vr.left-sr.left}px`;els.watermarkLayer.style.top=`${vr.top-sr.top}px`;els.watermarkLayer.style.width=`${vr.width}px`;els.watermarkLayer.style.height=`${vr.height}px`;
}
...
async function writeBlobToHandle(blob,outHandle,onProgress){
  const writable=await outHandle.createWritable();
  const reader=blob.stream().getReader();let written=0,total=blob.size||1;
  try{while(true){const {done,value}=await reader.read();if(done)break;await writable.write(value);written+=value.byteLength;onProgress?.(written/total);}await writable.close();}
    ...
    for(let i=0;i<queue.length;i++){
      ...
      try{
        ...
        if(state.watermarkEnabled&&state.watermarks.length){
          ...
          await writeRotatedSource(trimmed.blob,outHandle,item.rotation,within=>setOverallProgress(i,within,queue.length));
        }
        item.status='done';completed++;setOverallProgress(i,1,queue.length);
...
window.addEventListener('resize',()=>requestAnimationFrame(()=>{syncWatermarkLayerBounds();renderWatermarks();}));
