...
function outputName(item, extension = null) {
  const match = item.name.match(/^(.*?)(\.[^.]+)$/);
  const base = match ? match[1] : item.name;
  const ext = extension || (match ? match[2] : '.mp4');
...
function getTrimWorker() {
  ...
  worker.addEventListener('message', event => {
    ...
    if (msg.type === 'log') {
      if (msg.message) console.debug('[FFmpeg]', msg.message);
      return;
    }
    ...
    if (msg.ok) req.resolve(msg.buffer ? { buffer: msg.buffer, extension: msg.extension || '.mp4' } : true);
...
async function trimLossless(file, start, end) {
  const result = await workerRequest('trim', { file, start, end });
  const extension = result.extension || (/\.mov$/i.test(file.name) ? '.mov' : '.mp4');
  const mime = extension === '.mov' ? 'video/quicktime' : 'video/mp4';
  return { blob: new Blob([result.buffer], { type: mime }), extension };
}
  ...
  try {
    ...
    for (let i = 0; i < exportFiles.length; i++) {
      ...
      try {
        ...
        const trimmed = await trimLossless(file, start, end);
        source = trimmed.blob;
        const finalName = outputName(item, trimmed.extension);

        const outHandle = await state.outputDirHandle.getFileHandle(finalName, { create: true });
        els.processMessage.textContent = `กำลังบันทึก ${finalName} · Video-only Lossless (ไม่ Encode ภาพ)`;
