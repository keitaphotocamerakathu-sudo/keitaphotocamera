...
async function chooseOutputFolder(){
  if (!supported) return;
  try {
    // เลือกโฟลเดอร์ก่อนแบบ read เพื่อเลี่ยงบางกรณีที่ Chrome/macOS ปฏิเสธ picker เมื่อขอ readwrite ตั้งแต่แรก
    const opts={id:'sony-video-review-output'};
    if (state.dirHandle) opts.startIn=state.dirHandle; else opts.startIn='downloads';
    const h=await window.showDirectoryPicker(opts);
    await activateOutputHandle(h,{requestPermissionNow:true});
  } catch(e) {
    if(e?.name==='AbortError') {
      updateOutputPickerUI('ยกเลิกการเลือกโฟลเดอร์ หรือ Chrome ไม่อนุญาตโฟลเดอร์นั้น');
      return;
    }
