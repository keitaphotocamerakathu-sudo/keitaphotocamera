...
(() => {
  ...
  function l2norm(raw){
    let norm=0;for(const x of raw)norm+=x*x;norm=Math.sqrt(norm)||1;
    return raw.map(x=>x/norm);
  }
  async function embedPass(src,box,mirror=false){
    ...
    if(mirror){c.save();c.translate(112,0);c.scale(-1,1);c.drawImage(src,sx,sy,side,side,0,0,112,112);c.restore();}
    else{c.drawImage(src,sx,sy,side,side,0,0,112,112);}
    ...
    return Array.from(o.embedding.data);
  }
  async function embed(src,box){
    // Test-time augmentation used by FaceX's own LFW evaluation:
    // embed normal + horizontal flip, then L2-normalize the sum.
    // This reduces left/right asymmetry and improves strong profile matching
    // without changing detector thresholds or widening identity thresholds.
    const normal=await embedPass(src,box,false);
    const flipped=await embedPass(src,box,true);
    const sum=new Array(normal.length);
    for(let i=0;i<normal.length;i++) sum[i]=normal[i]+flipped[i];
    return l2norm(sum);
  }
  function boxIoU(a,b){
    ...
    const aa=Math.max(0,a.x2-a.x1)*Math.max(0,a.y2-a.y1),bb=Math.max(0,b.x2-b.x1)*Math.max(0,b.y2-b.y1);
    const u=aa+bb-inter;return u>0?inter/u:0;
  }
  function reliableDetections(items){
    const all=items||[];
    return all.filter((d,i)=>{
      // FaceX upstream demo uses 0.42. Accept those directly.
      if(Number(d.score||0)>=0.42) return true;
      // Low-score rescue is allowed only when the same spatial face is seen
      // again by another overlapping region. Foliage/road texture usually
      // appears only once and is rejected here.
      if(Number(d.score||0)<0.30) return false;
      return all.some((e,j)=>j!==i && e.source!==d.source && boxIoU(d,e)>=0.22);
    });
  }
  async function analyze(src,{deep=true}={}){
    await load(); const {w,h}=sourceSize(src); if(!w||!h) throw new Error("ขนาดภาพไม่ถูกต้อง");

    let dets=(await detectRegion(src,{x:0,y:0,w,h},.42))
      .map(d=>({...d,source:"full"}));

    if(deep && Math.max(w,h)>=1350){
      // 3x3 overlapping crops enlarge distant runners. The detector may use a
      // lower threshold on tiles, but low-score candidates need spatial
      // confirmation from another overlapping tile/full-frame pass.
      const frac=.46,tw=w*frac,th=h*frac;
      const xs=[0,Math.max(0,(w-tw)/2),Math.max(0,w-tw)];
      const ys=[0,Math.max(0,(h-th)/2),Math.max(0,h-th)];
      let ti=0;
      for(const y of ys){
        for(const x of xs){
          const region={x,y,w:tw,h:th};
          const found=await detectRegion(src,region,.28);
          dets=dets.concat(found.map(d=>({...d,source:"tile-"+(ti++)})));
        }
      }
    }

    dets=nms(reliableDetections(dets),.32);
  ...
  window.KeitaFaceV2={version:"facex-profile-v2",build:"20260921-profile2",load,analyze,cosineDistance};
})();