(() => {
  ...
  async function resizeAndOptimizeJpeg(item, changes) {
    ...
    try {
      const target = calcTargetDimensions(bitmap.width, bitmap.height, settings.longEdge, settings.noUpscale);
      if (!target) throw new Error('อ่านขนาดภาพไม่ได้');

      const canvas = createWorkCanvas(target.width, target.height);
      const ctx = canvas.getContext('2d', { alpha: false });
      if (!ctx) throw new Error('ไม่สามารถสร้าง Canvas สำหรับ Resize ได้');
      ...
      return {
        ...
        info: {
          ...
          reencoded: true,
          resized: target.width !== bitmap.width || target.height !== bitmap.height,
        },
      };
    } finally {
  ...
  async function canvasToJpegBlob(canvas, quality) {
    ...
    return new Promise((resolve, reject) => {
      canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Encode JPEG ไม่สำเร็จ')), 'image/jpeg', quality);
    });
  }
