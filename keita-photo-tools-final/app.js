(() => {
  ...
  const els = {
    ...
    exportBtn: $('exportBtn'), cancelBtn: $('cancelBtn'), exportProgressWrap: $('exportProgressWrap'), exportProgressBar: $('exportProgressBar'), exportProgressText: $('exportProgressText'), resultBox: $('resultBox'),
  };
  ...
  async function loadFiles(fileList) {
    ...
    state.items = files
      ...
      .map((file, index) => ({ file, index, meta: null, scanError: null, exportError: null }));
  ...
  function updatePreview() {
    ...
    if (!state.items.length) {
      els.previewBody.innerHTML = '<tr><td colspan="7" class="empty">เลือกรูปเพื่อดู Preview</td></tr>';
  ...
  function getValidation() {
    ...
    const anyOperation = els.renameEnabled.checked || els.dateFixEnabled.checked || els.timezoneFixEnabled.checked;
    ...
    if (!anyOperation) return { level: 'warn', blocking: true, messages: ['ยังไม่ได้เปิด Rename, Date Fix หรือ Time Zone Fix'] };
  ...
  async function exportAll() {
    ...
    state.items.forEach((item) => { item.exportError = null; });
    ...
    showResult(`เสร็จแล้ว: ${exportDirName} · สำเร็จ ${success.toLocaleString('th-TH')} · ผิดพลาด ${failed.toLocaleString('th-TH')} · ${extra.join(' · ')}${cancelledText}`, failed === 0);
    updatePreview();
  }
