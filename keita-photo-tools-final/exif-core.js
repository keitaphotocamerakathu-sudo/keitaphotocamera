(function (root, factory) {
  ...
  function parseTiffExif(view, bytes, tiffStart, segmentEnd, result) {
    ...
    result.orientation = readNumericEntry(view, tiffStart, ifd0.get(TAGS.Orientation), little) || 1;
  ...
  function hydrateFromTiff(view, bytes, tiffStart, segmentEnd, result) {
    ...
    bindNumeric(result, 'orientation', ifd0.get(TAGS.Orientation), little);
    result.orientation = readNumericEntry(view, tiffStart, ifd0.get(TAGS.Orientation), little) || result.orientation || 1;
    ...
    bindNumeric(result, 'pixelXDimension', exifIfd.get(TAGS.PixelXDimension), little);
    bindNumeric(result, 'pixelYDimension', exifIfd.get(TAGS.PixelYDimension), little);
  }
  ...
  function bindNumeric(result, locationKey, entry, little) {
    if (!entry || (entry.type !== 3 && entry.type !== 4) || entry.count < 1) return;
    result.locations[locationKey] = { offset: entry.valuePos, count: entry.count, type: entry.type, little: !!little };
  }
  ...
  function patchNumeric(arrayBuffer, changes) {
    const parsed = inspect(arrayBuffer);
    const bytes = new Uint8Array(arrayBuffer.slice(0));
    ...
    for (const [key, value] of Object.entries(changes || {})) {
      ...
      if (!loc || !Number.isFinite(n)) { skipped.push(key); continue; }
      if (loc.type === 3) view.setUint16(loc.offset, Math.max(0, Math.min(65535, Math.round(n))), !!loc.little);
      else if (loc.type === 4) view.setUint32(loc.offset, Math.max(0, Math.min(0xffffffff, Math.round(n))), !!loc.little);
  ...
  return { TAGS, inspect, patchAscii, patchNumeric };
});
